...
bot = telebot.TeleBot('7687232558:AAHxqNx7aNb-qzlc3gSgGb8F24jFZabYCxQ')
...
def log_command(user_id, target, port, time):
    ...
    if user_info.username:
        username = "@vort33x" + user_info.username
    else:
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "❌ Unauthorized! Only admin can use this command. Buy access: @PRIME_X_ARMY"
...
# Function to handle the attack reply
def start_attack_reply(message, target, port, time):
    user_info = message.from_user
    username = user_info.username if user_info.username else user_info.first_name
    
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.🚀🚀\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP - @PRIME_X_ARMY 🔥"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        if user_id not in admin_id:
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 0:
                response = "You Are On Cooldown ❌. Please Wait Before Running The /attack Command Again."
                bot.reply_to(message, response)
                return
...
def welcome_start(message):
    ...
    response = f'''❄️ Welcome To Premium DDOS Bot, {user_name}! This is high quality server based DDOS. To get access:
...
2. Don't Run 2 Attacks At Same Time Otherwise You Will Be Banned
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, Only 1 Plan Is Powerful Than Any Other DDOS !!:
...
💥 /add <userId> <duration> : Add a User with time limit
💥 /remove <userid> : Remove a User
💥 /allusers : Authorised Users List
💥 /logs : All Users Logs
